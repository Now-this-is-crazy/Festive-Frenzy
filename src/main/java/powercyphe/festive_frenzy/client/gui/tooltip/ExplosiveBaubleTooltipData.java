...
public class ExplosiveBaubleTooltipData implements TooltipComponent, ClientTooltipComponent {
    ...
    public int getHeight(Font font) {
        return this.shouldShowTooltip() ? HEIGHT + 2 : 0;
    }

    @Override
    public int getWidth(Font font) {
        return this.shouldShowTooltip() ? WIDTH + 2 : 0;
    }
    ...
    public void renderImage(Font font, int x, int y, int width, int height, GuiGraphics guiGraphics) {
        if (this.shouldShowTooltip()) {
            guiGraphics.blit(RenderType::guiTextured, DETAILS_TEXTURE, x, y, 0, 0, 16, 24, 16, 24);
    ...
    private Component getExplosionPowerText() {
        return Component.translatable("festive_frenzy.explosion_power")
                .append(Component.translatable("festive_frenzy.explosion_power." + this.explosionPower));
    }

    private Component getExplosionModificationText() {
        return Component.translatable("festive_frenzy.explosion_modification")
