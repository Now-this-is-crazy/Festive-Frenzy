...
import powercyphe.festive_frenzy.block.property.ExplosionModificationProperty;
...
public class BaubleBlock extends Block {
    ...
    public BaubleBlock(Settings settings) {
        super(settings);
        this.setDefaultState(this.getDefaultState().with(GLOWING, false).with(EXPLOSION_STRENGTH, 0).with(EXPLOSION_MODIFICATION, BaubleBlockItem.ExplosionModification.NONE));
    }
    ...
    public List<ItemStack> getDroppedStacks(BlockState state, LootContextParameterSet.Builder builder) {
        ...
        BaubleBlockItem.ExplosionModification modification = state.get(EXPLOSION_MODIFICATION);

        if (explosionStrength > 0)
        for (int i = 0; i < stacks.size(); i++) {
            ItemStack stack = stacks.get(i);
            if (stack.isIn(ModTags.Items.BAUBLES_TAG)) {
                stack.getOrCreateNbt().putInt(BaubleBlockItem.EXPLOSION_STRENGTH_KEY, explosionStrength);
                stack.getOrCreateNbt().putString(BaubleBlockItem.EXPLOSION_MODIFICATION_KEY, modification.getName());
                stacks.set(0, stack);
            }
        }
        return stacks;
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        world.setBlockState(pos, state.with(EXPLOSION_STRENGTH, itemStack.getOrCreateNbt().getInt(BaubleBlockItem.EXPLOSION_STRENGTH_KEY))
                .with(EXPLOSION_MODIFICATION, BaubleBlockItem.ExplosionModification.fromName(itemStack.getOrCreateNbt().getString(BaubleBlockItem.EXPLOSION_MODIFICATION_KEY))));
